...
from utils.errors import BankTypeError, FilelrdErrorX
...
def upload_questions(file_path=None, bank_info=None):
    ...
    for i in range(0, nrows):
        rvaluesx = table.row_values(i)  # 获取行中的值
        break
    t_rvaluesx= rvaluesx[0].strip()
    print("t_rvaluesx="+t_rvaluesx)
    if t_rvaluesx != "问题":
        return choice_num, fillinblank_num

    for i in range(1, nrows):
        rvalues = table.row_values(i)  # 获取行中的值
        print("获取行中的值rvalues[0]="+rvalues[0])
