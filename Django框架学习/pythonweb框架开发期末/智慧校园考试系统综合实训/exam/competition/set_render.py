...
from utils.errors import (FileNotFound, FileTypeError, ProfileNotFound,
                          TemplateNotFound, BizAccountNotFound, BankTypeError, FilelrdErrorX, BankNameNotFound)
...
def upload_bank(request):
    ...
    for k, v in dict(BankInfo.BANK_TYPES).items():
        if v == bank_type:
            bank_type = k
            break
    bank_names=bank_name.replace(" ", "")
    if bank_names == "":
        print(bank_name)
        return render(request, 'err.html', BankNameNotFound)
    if not template:  # 模板不存在
        return render(request, 'err.html', FileNotFound)
    print("bank_type="+str(bank_type))
    if bank_type is None:
        return render(request, 'err.html', BankTypeError)
    if str(bank_type) == '选择一个题库类型':
        return render(request, 'err.html', BankTypeError)
    ...
    if not os.path.exists(today_bank_repo):
        os.makedirs(today_bank_repo, exist_ok=True)  # exist_ok=True 表示目录已存在时不报错
    ...
    if choice_num==0 and fillinblank_num==0:
        return render(request, 'err.html', FilelrdErrorX)
