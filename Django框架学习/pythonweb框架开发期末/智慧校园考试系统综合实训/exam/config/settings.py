...
TEMPLATES = [
    {
        'BACKEND': 'django.template.backends.django.DjangoTemplates',
        'DIRS': [os.path.join(WEB_DIR, 'templates'), ],
        'APP_DIRS': True,  # 需要设置为True才能使用{% static %}标签
        'OPTIONS': {
            'context_processors': [
                'django.template.context_processors.debug',
                'django.template.context_processors.request',
                'django.contrib.auth.context_processors.auth',
                'django.contrib.messages.context_processors.messages',
                'django.template.context_processors.static',  # 添加这个
            ],
            # 移除或注释掉 loaders 配置，让Django使用默认加载器
            # 'loaders': [
            #     ('django.template.loaders.cached.Loader', [
            #         'django.template.loaders.filesystem.Loader',
            #         'django.template.loaders.app_directories.Loader',
...
DATABASES = {
    'default': {
        ...
        'OPTIONS': {
            ...
            # 设置默认行格式
            'init_command': "SET default_storage_engine=INNODB; SET GLOBAL innodb_file_format=Barracuda;",
        },
    }
}
...
# 网站信息
SITE_URL = 'http://127.0.0.1:8000'  # 你的域名
SITE_NAME = 'Quizz.cn'
SUPPORT_EMAIL = 'support@quizz.cn'
