...
from logger.logging_configurator import create_logs_path
...
class LoggerManager:
    ...
    def use_logging_config(self, config_manager) -> bool:
        ...
        if config_manager.success_flag is False and hasattr(config_manager.logging_configurator, "error_msg"):
            ...
            # 确保自定义日志配置的日志输出目录存在
            mkdir(Path(config_manager.log_config["handlers"]["file"]["filename"]).parent)
            mkdir(Path(config_manager.log_config["handlers"]["error_file"]["filename"]).parent)
            # 更新日志记录器(全局处理后跟着文件和控制台输出的处理器)
            logging.config.dictConfig(config_manager.log_config)
            # 更新日志记录器
            self.logger = logging.getLogger(LOGGER_NAME)
            # 安全删除默认配置(释放默认配置的空间)
    ...
    def update_logging_config(self, log_config: dict) -> bool:
        """使用自定义的日志配置更新当前日志管理器
        使用新的配置重新配置日志记录器。
        :param log_config: 配置管理器实例,日志配置信息
