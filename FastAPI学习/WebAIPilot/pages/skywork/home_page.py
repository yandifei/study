...
class HomePage(BasePage):
    ...
    async def _async_init(self):
        ...
        """
        # 访问天工对话网页
        await self.page.goto(self.cm.config_data["AI"]["skywork"]["chat_url"])
    ...
    #     # 输入内容
    #     await self.page.get_by_placeholder("请输入需求，输入@可引用文件").fill(question)
    ...
    #     # 同步获取捕获到的变量，evaluate 会等待并返回结果
    #     text_answer = await self.page.evaluate("window.tempCopyBuffer")
    #     debug(f"最后对话中天工回复的内容: {text_answer}")
    #     return text_answer
    #
