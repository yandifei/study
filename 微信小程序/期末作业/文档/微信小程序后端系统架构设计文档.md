...
*   **内网穿透隧道 (Tunnel):** ngrok 客户端运行在宿主机，将本地 Nginx 暴露的 61000 端口映射至公网 HTTPS 地址。
...
*   **核心职责:**
    ...
    *   **统一入口 (Single Entry Point):** 对外仅暴露 8000 端口（映射至宿主机 61000），隐藏后端服务的实际端口，简化客户端调用和 ngrok 隧道配置。
...
*   **设计原理:**
    1.  **统一入口:** Nginx 对外仅暴露 8000 端口（映射至宿主机 61000），作为所有 API 请求的唯一入口。
...
4.  **Nginx 容器化部署（可选，单服务开发时可跳过）:** 编写 `docker-compose.yml` 文件，拉取 `nginx:stable-alpine` 镜像，挂载 `nginx.conf`，将容器 8000 端口映射至宿主机 61000 端口。
5.  **后端服务启动:** 
    *   数据库服务：`python main.py`（Uvicorn，端口 21325）
    *   AI 服务：单独启动（端口 21326，独立项目）
6.  **公网暴露:** 在宿主机上运行 `ngrok` 客户端，将流量指向本地 61000 端口（Nginx 入口），或直接指向 21325 端口（单服务调试）。
...
flowchart LR
    ...
    subgraph LocalHost [本地开发主机 - Windows]
        subgraph Docker [Docker 容器]
            NginxContainer[容器: Nginx<br/>nginx:stable-alpine<br/>内部 8000 → 宿主机 61000]
        end
    ...
    User -->|HTTPS 请求| Ngrok
    Ngrok -->|隧道转发 localhost:61000| NginxContainer
...
- **ngrok** 运行在宿主机，将 Nginx 的 61000 映射端口暴露到公网 HTTPS 地址。
