...
function deleteProvider(obj){
	 $.ajax({
	 	...
	 	data:{method:"delprovider",providerid:obj.attr("providerid")},
	 	dataType:"json",
	 	success:function(data){
	 		if(data.delResult == "true"){//删除成功：移除删除行
	 			...
	 			changeDLGContent("对不起，删除供应商【"+obj.attr("proContactname")+"】失败");
	 		}else if(data.delResult == "notexist"){
	 			// alert("对不起，供应商【"+obj.attr("proname")+"】不存在");
	 			changeDLGContent("对不起，供应商【"+obj.attr("proContactname")+"】不存在");
	 		}else{
	 			// alert("对不起，该供应商【"+obj.attr("proname")+"】下有【"+data.delResult+"】条订单，不能删除");
	 			changeDLGContent("对不起，该供应商【"+obj.attr("proContactname")+"】下有【"+data.delResult+"】条订单，不能删除");
	 		}
	 	},
	 	error:function(data){
	 		//alert("对不起，删除失败");
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/providerview?providerid="+ obj.attr("providerid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/providermodify?providerid="+ obj.attr("providerid");
	});
	...
	$(".deleteProvider").on("click",function(){
		providerObj = $(this);
		changeDLGContent("你确定要删除供应商【"+providerObj.attr("proContactname")+"】吗？");
		openYesOrNoDLG();
	});
