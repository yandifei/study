...
def qq_quick_order(order: str):
    ...
    function_map = {
        ...
        "#温度": [lambda : deepseek.set_temperature(),f"已修改温度为{deepseek.temperature}", "超出温度范围([0.0-2.0]分,默认1.0)"],
        ...
        "#位置输出概率": [lambda : deepseek.set_top_logprobs(), f"已修改概率输出个数为{deepseek.top_logprobs}", "未开启对数概率输出或参数不在调用范围(0-20)"],  # int(input("请指定的每个输出位置返回输出概率top为几的token(0-20，默尔为None):"))
        # FIM对话参数
        "#补全开头": [lambda : deepseek.set_prompt(args),"已补全开头", "补全开头失败"],
        "#完整输出": [lambda : deepseek.set_echo(),"因为服务器那边不接受True，只接受False和None,所以这个功能无效", "因为服务器那边不接受True，只接受False和None,所以这个功能无效"],  # 这个参数就只有False和None了，改不了一点
        "#FIM对数概率输出": [lambda : deepseek.set_FIM_logprobs(),f"已制定输出中保留{deepseek.FIM_logprobs}个最可能输出token的对数概率", "参数不在调用范围(0-20)"],  # int(input("请输入需要多少个候选token数量输出对数概率(默认0):"))
        ...
        "#聊天记录": [lambda : True if len(deepseek.dialog_history) > 0 else False, lambda : deepseek.print_dialog_history(),"聊天记录为空"],
        "#清空对话历史": [lambda : deepseek.clear_dialog_history(),"已清空对话历史(人设除外)","对话历史为空无需清空"],
        # 多人设管理
        "#人设切换": [lambda : deepseek.role_switch(args),f"已切换人设为：{args}","提示库不存在该人设"],    # input("请输入切换的人设:")
        "#所有人设": [lambda : deepseek.role_list(),"提示库的所有人设:" + " ".join(deepseek.role_list()), "提示库中为空，不存在任何人设"],  # 人设做了处理
        "#人设查询": [lambda : deepseek.select_role_content(args),lambda : deepseek.select_role_content(args), "不存在该人设，无法进行打印"],    # input("请输入要查询的人设:")
        "#当前人设": [lambda : deepseek.print_role_content(),f"当前人设:{deepseek.role}","当前人设为空"],
        "#人设自定": [lambda : deepseek.set_role(args),"自定义人设成功", "自定义人设失败"],# input("请输入人设内容:")
        ...
        "#toekn": [True, lambda : deepseek.return_token(), lambda : deepseek.return_balance(), "无法查询"]
    }
